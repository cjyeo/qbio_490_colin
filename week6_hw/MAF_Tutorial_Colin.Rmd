...
TP53 is the gene I chose. This gene is responsible for the synthesis of tumor protein p53, which acts as a tumor suppressor (it regulats cell division).
...
```{r}
oncoplot(maf = maf_object,
         genes = c("TP53", "TTN", "RYR2", "KMT2C", "GATA3"))
```
...
1. Are TP53 and RYR2 mutations linked in some way, or are they mutually exclusive? They both have a high rate of missense-mutation, compared to the other genes.
2. What similarities do GATA3 and KMT2C have in their respective biological pathways? They don't seem to be linked by a specific mutation type.
...
Yes -- the bottom row labeled "age_category" shows that there is some grouping of red and blue areas.
...
```{r}
 age_mask <- with(maf_object@clinical.data, ifelse(maf_object@clinical.data$age_category == "young", TRUE, FALSE))

young_patient_barcodes <- maf_object@clinical.data$Tumor_Sample_Barcode[age_mask]
...
```{r}
old_patient_barcodes <- maf_object@clinical.data$Tumor_Sample_Barcode[!age_mask]
...
```{r}
jpeg("/Users/colinyeo/desktop/School/qbio_490_colin/week6_hw/co0ncoplot_young_old.png")
...
Hypothesized Reason: The types of mutations associated with the PIK3CA gene tend to accumulate over time, which would mean that older individuals probably have more of these mutations than younger individuals, which lead to abnormal cell production.
...
```{r}
jpeg("/Users/colinyeo/desktop/School/qbio_490_colin/week6_hw/lollipop_TP53")
...
Yes, P53_TAD (responsible for responses to acute DNA damage), P53 (site of the gene), and P53_tetramer (essential for DNA binding and protein interactions)
...
Around #175-180, and there are about 20 mutations at this location.
...
It looks like the gene is more commonly mutated in younger patients.

Are there more mutations in a specific region of the protein? Why might that be?
The P53 region of the protein has the most mutation. This might be because a direct mutation of the protein/gene directly causes negative impacts (i.e. unchecked cell proliferation) more than the other two domains (which may indirectly impact cell proliferation).

What kinds of mutations are the most common?
Missense mutations.

Are there any other interesting observations?
There are some mutations that exist outside of the three established domains , but they don't happen as frequently as the ones within the domains.
...
```{r}
maf_object@clinical.data$survival_time <- with(maf_object@clinical.data, ifelse(is.na(maf_object@clinical.data$days_to_death), maf_object@clinical.data$days_to_last_followup, maf_object@clinical.data$days_to_death))

print(sum(is.na(maf_object@clinical.data$survival_time)))
      
maf_object@clinical.data$death_event <- with(maf_object@clinical.data, ifelse(maf_object@clinical.data$vital_status == "Alive", FALSE, TRUE))
...
The mutant type of my chosen gene shows a lower survival probability from 0 to 4000 on the time scale, but evens out with the wild type after that.

What can you conclude from your plot?
TP53 mutations will probably be more present in younger individuals (because it has a lower survival rate early-on, then evens out at the end)

Is this what you would expect according to the literature? Why or why not?
Yes, as this is also reflected in our plot (where TP53 shows a higher incidence of mutation).
