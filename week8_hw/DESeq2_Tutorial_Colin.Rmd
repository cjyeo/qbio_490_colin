...
# loaded in rna_se from the .RData file
rna_clinical <- read.csv("/Users/colinyeo/desktop/School/qbio_490_colin/analysis_data/brca_rna_clinical_data.csv", header = TRUE, row.names = 1)
rna_genes <- read.csv("/Users/colinyeo/desktop/School/qbio_490_colin/analysis_data/brca_rna_gene_data.csv", header = TRUE, row.names = 1)
rna_counts <- read.csv("/Users/colinyeo/desktop/School/qbio_490_colin/analysis_data/brca_rna_count_data.csv", header = TRUE, row.names = 1)
```
...
List your chosen variable here:rna_clinical$definition
...
# need to subset out the first column (with the names of the rows)
row_sums <- rowSums(rna_counts[ ,2:1101])
...
ordered_indices <- order(-up_reg_results$y)

up_reg_results <- up_reg_results[ordered_indices, ]
```
...
```{r}
doubleMask <- ifelse(up_reg_results$y....results.log2foldchange.sig_results. > 1, T, F)
up_reg_results <- up_reg_results[doubleMask, ]
```
...
down_ordered_indices <- order(down_reg_results$y)

down_reg_results <- down_reg_results[down_ordered_indices, ]
negDoubleMask <- ifelse(down_reg_results$y....results.log2foldchange.sig_results. < -1, T, F)
down_reg_results <- down_reg_results[negDoubleMask, ]
```
...
```{r}
row.names(up_reg_results) <- c(gsub(" ", "", paste("gene_number_", 1:124)))
row.names(down_reg_results) <- c(gsub(" ", "", paste("gene_number_", 1:274)))
```
...
Gene ensembl id: FGB
Log2FoldChange: -2.83
p-adj value: 6.8 * 10^-20
More highly expressed in young or old patients: Old
Google the gene. What is its full name, and what appears to be its function? Genecards generally has a pretty good summary: Fibrinogen Beta Chain, function is related to cell adhesion and blood clotting. 

Gene ensembl id:FDCSP
Log2FoldChange:2.05
p-adj value: 7.97 * 10^-17
More highly expressed in young or old patients: Young
Google the gene. What is its full name, and what appears to be its function? Genecards generally has a pretty good summary:Follicular Dendritic Cell Secreted Protein, function is related to the metastasis of tumors and cancer cell migration.

Write the results, up_reg_results, and down_reg_results df's to analysis_data using write.csv().

```{r}
write.csv(results, "/Users/colinyeo/desktop/School/qbio_490_colin/analysis_data/brca_deseq2_results.csv", row.names = TRUE)
write.csv(up_reg_results, "/Users/colinyeo/desktop/School/qbio_490_colin/analysis_data/brca_deseq2_up_results.csv", row.names = TRUE)
write.csv(down_reg_results, "/Users/colinyeo/desktop/School/qbio_490_colin/analysis_data/brca_deseq2_down_results.csv", row.names = TRUE)
```
...
top-middle genes: significant, same expression
